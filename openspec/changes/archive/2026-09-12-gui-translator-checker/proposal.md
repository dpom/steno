...
- A single-window Tkinter screen following the mockup (`specs/translator-checker/GUI-screen.png`): top panel = large central image frame labelled "gliph image"; middle panel = table named "lsigns" with 6 columns (Lsign image thumbnails, freq char + float score, diff char + float score, knn char + float score, select input char, save checkbox); bottom diamond control cluster = SAVE on top, PREV/NEXT left/right, QUIT at bottom.
- A Save action writes every row whose save box is checked and whose letter is non-empty as `<image>-<row>-<col>-<pos>.edn` under `<corpus-dir>/<letter>/` using the exact corpus format of `action-make-corpus` (`ltype`, `lineseq`, `fileimage`, `row`, `column`, `pos` — no translator annotations); unchecked or empty rows are skipped, and typing a letter with no folder yet creates it on save.
